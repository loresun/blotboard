 ...
 */
/**
 * 通用服务账号名：CI runner、容器、云主机镜像里的系统用户名。
 *
 * 它们不是任何人的私人标识，却极可能撞上代码里的普通词。这个仓库真的栽过：
 * GitHub Actions 的系统用户名就叫 `runner`，而 Runner 恰好是本项目的核心概念
 * （任务后端）——第一次上 CI，全仓 239 处 "runner" 被一口气判成「作者身份泄漏」，
 * `npm run check` 红在第一步，后面五步一步没跑。
 *
 * 系统账号名本来就只是「作者是谁」的**启发式猜测**（真正可靠的是 git config 里的身份）。
 * 猜到这一类通用名就该弃用：留着它一条真问题也查不出，只会把 CI 变成必红。
 */
const GENERIC_ACCOUNTS = new Set([
  "runner", "root", "admin", "administrator", "user", "users", "guest", "default",
  "ubuntu", "debian", "centos", "alpine", "ec2-user", "azureuser",
  "build", "builder", "jenkins", "github", "gitlab", "actions", "circleci", "travis", "vsts",
  "docker", "container", "node", "vscode", "codespace",
]);
...
function identityPatterns() {
  ...
  try {
    // 系统账号名是猜的，通用服务账号（CI runner / 容器 / 云镜像）直接弃用，见 GENERIC_ACCOUNTS
    for (const value of [os.userInfo().username, path.basename(os.homedir())]) {
      if (value && !GENERIC_ACCOUNTS.has(value.toLowerCase())) raw.add(value);
    }
  } catch {
    /* 某些容器里拿不到，跳过 */
  }
