...
function useCompactLevel(ref: React.RefObject<HTMLElement | null>, signature: string) {
  ...
  //
  // 判定对溢出**零容忍**（`>` 而不是 `> +1`）：这里曾经留过 1px 容差，结果是
  // 「差一点点装不下」时它认为装得下，于是停在当前档，让最右边那颗按钮探出可视区 1px。
  // 本机看不出来——字体宽度按平台差一点点，同一档在 macOS 上量出 900、在 Linux 上量出 901，
  // 第一次上 CI 才炸出来（900px 视口，档位停在 4，「更多」右边越界）。
  // 多收一档的代价只是少露一颗按钮（它在「更多」里一件不少），比溢出便宜得多。
  useLayoutEffect(() => {
    const bar = ref.current;
    if (!bar) return;
    if (level < COMPACT_MAX && bar.scrollWidth > bar.clientWidth) {
